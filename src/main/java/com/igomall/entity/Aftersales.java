...
public class Aftersales extends BaseEntity<Long> {
	...
	public enum Type {
		 ...
		 * @return 类型
		 */
		public static Type getType(final Class<? extends Aftersales> clazz) {
			Assert.notNull(clazz, "[Assertion failed] - clazz is required; it must not be null");

			return (Type) CollectionUtils.find(Arrays.asList(Type.values()), new Predicate() {
